...
changes carry over to the new branch. Commit or stash first if you want the new layer to start
clean.
...
**Use a single stack** when every branch serves the same feature or project, even if the layers
span different concerns.
...
A trivial incidental fix can ride along in the current stack. Once it grows into its own project,
it deserves its own stack.
