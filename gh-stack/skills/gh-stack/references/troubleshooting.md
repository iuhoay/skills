...
first, so a failed `sync` leaves nothing half-applied; a failed `rebase` stops mid-flight and
waits.
...
time the same conflict appears — which is common, since a change low in the stack is rebased
through every branch above it. Without `rerere`, repeated conflicts may need manual resolution on
each affected layer.
...
and then `--continue` until complete. Use `gh stack sync --prune` to also delete local branches
for merged PRs.
...
`init` adopts branches that already exist, so the rebuild reuses them rather than creating new
ones. Existing PRs survive. Once Git ancestry is correct, `submit` updates their base branches and
...
Commands exit 6 when the current branch cannot identify a single stack — typically because it is
the trunk of more than one stack. There is no flag to disambiguate.
...
will not work on the result. Use `gh stack checkout <stack-number>` if you later want local
tracking.
