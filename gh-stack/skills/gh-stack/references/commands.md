...
branch immediately before it. There is no separate adopt mode — existence decides (the legacy
`--adopt` flag is deprecated). `--base` selects a non-default trunk; `--prefix X --numbered`
auto-names branches.
...
- **Not atomic.** Branches are pushed sequentially with per-branch `--force-with-lease`. If a later
  push is rejected, earlier pushes and PR updates stand. Fix the rejection and rerun the same
  command.
- **A fully merged stack cannot be extended.** When every PR in the current stack is already
  merged, `submit` forks the remaining unmerged branches into a **new** stack rooted at the trunk
  and creates it on GitHub, leaving the merged stack untouched.
...
7. **Sync the stack object** — link open PRs into a stack, additively. Only when two or more PRs
   exist. `sync` never opens PRs; that is `submit`. "Stack synced" means the remote stack object
   matches local; "Branches synced" means branches were rebased/pushed but no remote stack exists
   yet.
8. **Prune** local branches for merged PRs, only when `--prune` is passed in a non-interactive
   environment. If the checked-out branch would be pruned, you are moved to the first active
   branch or the trunk.
...
- `--no-trunk` skips fetching and the trunk rebase entirely, aligning stack branches with each
  other only.
- `[branch]` starts the cascade from that branch instead of the bottom.
- `--preserve-dates` keeps author dates on rewritten commits.
...
- With a stack number it works from anywhere in the repository, tracked locally or not, via the
  API. Local tracking is also removed when present.
...
- Blocked while every PR in the stack is queued for merge, merging, or already merged.
...
- **A merge queue on the base branch overrides everything.** The stack is added to the queue rather
  ...
  warning. Queued PRs are submitted together but land as the queue processes them, so they may
  merge in separate groups rather than all at once.
